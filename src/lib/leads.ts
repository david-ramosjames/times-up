 ...
 *        LEAD_FROM_EMAIL  — a verified sender, e.g. "Times Up <leads@timesuplaw.com>"
...
async function sendEmail(
  ...
  const from =
    ...
    "Times Up Website <leads@timesuplaw.com>";
  ...
  const html = `<div style="font-family:Arial,Helvetica,sans-serif;max-width:640px">
    <h2 style="color:#6D28D9;margin:0 0 14px">${escapeHtml(subject)}</h2>
    <table style="border-collapse:collapse;width:100%">${rows}</table>
    <p style="margin-top:18px;color:#999;font-size:12px">Sent automatically from timesuplaw.com</p>
  </div>`;
    ...
    const res = await fetch(webhook, {
      ...
      body: JSON.stringify({
        ...
        blocks: [
          {
            type: "header",
            text: { type: "plain_text", text: `⚖️ ${subject}`, emoji: true },
          },
          { type: "section", text: { type: "mrkdwn", text: body } },
          {
            type: "context",
            elements: [
              { type: "mrkdwn", text: "Submitted on timesuplaw.com" },
            ],
          },
        ],
      }),
