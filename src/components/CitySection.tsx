  ...
  return (
    <article className="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm transition-shadow hover:shadow-md">
      ...
      <h2 className="text-2xl font-bold text-brand-navy">
        {locale === "en"
          ? `${city} Sexual Assault Lawyer`
          : `Abogada de Agresión Sexual en ${city}`}
      </h2>
      ...
      <div className="mt-6 grid gap-6 sm:grid-cols-2">
        ...
        <div>
          <h3 className="text-sm font-bold uppercase tracking-wider text-brand-navy">
            {locale === "en"
              ? `Where institutions fail survivors in ${city}`
              : `Dónde fallan las instituciones en ${city}`}
          </h3>
        ...
        <div>
          <h3 className="text-sm font-bold uppercase tracking-wider text-brand-navy">
            {locale === "en"
              ? `Cases we handle in ${city}`
              : `Casos que manejamos en ${city}`}
          </h3>
          <ul className="mt-2 space-y-1.5">
      ...
      <p className="mt-5 text-sm text-brand-steel">
        {locale === "en" ? (
          <>
            Our <Link href={routes.sexualAssault} className="font-medium text-brand-coral underline hover:text-brand-rose">sexual assault lawyers</Link> investigate
            the people and institutions responsible — confidentially and at no upfront cost. Have questions?{" "}
            <Link href={routes.faq} className="font-medium text-brand-coral underline hover:text-brand-rose">See our FAQ</Link>.
          </>
        ) : (
          <>
            Nuestras <Link href={routes.sexualAssault} className="font-medium text-brand-coral underline hover:text-brand-rose">abogadas de agresión sexual</Link> investigan
            a las personas e instituciones responsables — de forma confidencial y sin costo inicial. ¿Tienes preguntas?{" "}
      ...
      <div className="mt-5">
        ...
        >
          ...
          <span className="lg:hidden">{locale === "en" ? "Call Now — Confidential" : "Llamar Ahora — Confidencial"}</span><span className="hidden lg:inline">{dict.cta.callNow}: {PHONE_DISPLAY}</span>
        </a>
        {routeKey && (
          <Link
