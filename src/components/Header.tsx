...
export default function Header({ dict, locale }: { dict: Dictionary; locale: Locale }) {
  ...
  const caseDropdownItems = [
    { href: routes.sexualAbuse, label: isEn ? "Sexual Abuse & Childhood Abuse" : "Abuso Sexual e Infantil" },
    { href: routes.rideshare, label: isEn ? "Rideshare (Uber/Lyft) Assault" : "Agresión en Uber/Lyft" },
    { href: routes.hotelAssault, label: isEn ? "Hotel & Negligent Security" : "Agresión en Hoteles" },
    { href: routes.workplaceAssault, label: isEn ? "Workplace Sexual Assault" : "Agresión en el Trabajo" },
    { href: routes.schoolAbuse, label: isEn ? "School & Campus Abuse" : "Abuso en Escuelas" },
    { href: routes.sexualAssault, label: isEn ? "All Sexual Assault Cases" : "Todos los Casos" },
  ];
  ...
  const helpDropdownItems = [
    ...
    { href: routes.helpCanISue, label: isEn ? "Can I Sue My Abuser?" : "¿Puedo Demandar al Agresor?" },
    { href: routes.helpStatuteOfLimitations, label: isEn ? "Texas Filing Deadlines" : "Plazos para Demandar en Texas" },
    { href: routes.helpSueInstitution, label: isEn ? "Sue a Hotel, Employer, or School" : "Demandar a una Institución" },
    { href: routes.helpAnonymous, label: isEn ? "Can I Stay Anonymous?" : "¿Puedo Permanecer Anónima?" },
    { href: routes.helpCivilVsCriminal, label: isEn ? "Civil vs. Criminal Case" : "Caso Civil vs. Penal" },
    { href: routes.helpCaseWorth, label: isEn ? "How Much Is My Case Worth?" : "¿Cuánto Vale Mi Caso?" },
    { href: routes.helpCompensation, label: isEn ? "What Can I Recover?" : "¿Qué Compensación Puedo Recibir?" },
    { href: routes.helpAverageSettlement, label: isEn ? "Average Settlement" : "Acuerdo Promedio" },
    { href: routes.helpNeverReported, label: isEn ? "What If I Never Reported?" : "Nunca lo Denuncié" },
    { href: routes.help, label: isEn ? "All Survivor Help" : "Toda la Ayuda" },
  ];
  ...
  const casePaths: string[] = caseDropdownItems.map((i) => i.href);
  const isCasePage = casePaths.includes(pathname);
        ...
        <div className="flex items-center justify-between border-b border-white/10 px-4 py-2">
          ...
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2 text-sm font-bold uppercase tracking-wider">
              ...
              <span className="text-brand-red">
                {locale === "en" ? "Confidential & Available 24/7" : "Confidencial y Disponible 24/7"}
              </span>
            </div>
            ...
            <div className="group relative">
              <button
                className={`flex items-center gap-1 whitespace-nowrap text-[13px] font-medium leading-tight transition-colors hover:text-brand-red ${
                  isCasePage ? "text-brand-red" : ""
                }`}
              >
                {isEn ? "Cases We Handle" : "Casos"}
        ...
        <nav className="max-h-[80vh] overflow-y-auto border-t border-white/10 bg-brand-navy-dark lg:hidden" aria-label="Mobile navigation">
          <div className="mx-auto max-w-7xl space-y-1 px-4 py-4">
            ...
            <div className="rounded px-3 py-2 text-sm font-bold uppercase tracking-wider text-gray-400">
              {isEn ? "Cases We Handle" : "Casos que Manejamos"}
            </div>
            {caseDropdownItems.map((item) => (
              <Link
                key={item.href}
            ...
            <div className="rounded px-3 py-2 text-sm font-bold uppercase tracking-wider text-gray-400">
              {isEn ? "Survivor Help" : "Ayuda para Sobrevivientes"}
            </div>
            {helpDropdownItems.map((item) => (
              <Link
                key={item.href}
            ...
            >
              ...
              <span className="lg:hidden">{locale === "en" ? "Call Now — Confidential" : "Llamar Ahora — Confidencial"}</span><span className="hidden lg:inline">{dict.nav.callNow}: {PHONE_DISPLAY}</span>
            </a>
          </div>
        </nav>
      )}
