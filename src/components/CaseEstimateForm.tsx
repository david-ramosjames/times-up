...
export default function CaseEstimateForm({
      ...
      <div data-form-success="case-estimate">
        ...
        <div className="mb-6 rounded-xl bg-brand-navy p-6 text-center text-white shadow-lg md:p-7">
          <h3 className="text-xl font-bold md:text-2xl">
            {locale === "en"
              ? "Talk to a Real Person — Free & Confidential"
              : "Habla con una Persona Real — Gratis y Confidencial"}
          </h3>
          <p className="mx-auto mt-2 max-w-xl text-gray-300">
            {locale === "en"
              ? "This is only a preliminary review. Get a free, confidential, no-obligation conversation with our team."
              : "Esta es solo una revisión preliminar. Recibe una conversación gratuita, confidencial y sin compromiso con nuestro equipo."}
          </p>
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-8">
      ...
      {/* Case Type */}
      <fieldset>
        <legend className="mb-3 text-lg font-bold text-brand-navy">{f.caseTypeLabel}</legend>
        <div className="grid gap-2 sm:grid-cols-2">
          {[
            { value: "assault", label: f.caseTypeAssault },
            { value: "abuse", label: f.caseTypeAbuse },
            { value: "rideshare", label: f.caseTypeRideshare },
            { value: "workplace", label: f.caseTypeWorkplace },
            { value: "institution", label: f.caseTypeInstitution },
            { value: "other", label: f.caseTypeOther },
          ].map((o) => (
            <label
              key={o.value}
              className={`flex cursor-pointer items-center gap-3 rounded-lg border px-4 py-3 transition-colors ${
      ...
      <fieldset>
        ...
        <div className="grid gap-2 sm:grid-cols-2">
          {[
            { value: "self", label: f.roleSelf },
            { value: "child", label: f.roleChild },
            { value: "family", label: f.roleFamily },
            { value: "friend", label: f.roleFriend },
          ].map((o) => (
            <label
              key={o.value}
              className={`flex cursor-pointer items-center gap-3 rounded-lg border px-4 py-3 transition-colors ${
