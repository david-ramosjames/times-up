...
export const metadata: Metadata = {
  title: "Hotel Sexual Assault & Negligent Security Lawyer in Texas",
  description:
  ...
  alternates: {
    canonical: `${SITE_URL}/hotel-assault-negligent-security-lawyer`,
    languages: {
      en: `${SITE_URL}/hotel-assault-negligent-security-lawyer`,
      es: `${SITE_URL}/es/abogada-agresion-sexual-hotel`,
    },
  },
};

export default function HotelAssaultPage() {
  const dict = getDictionary("en");
  const d = dict.hotelAssault;
  const routes = ROUTES.en;

  return (
    <PageShell dict={dict} locale="en">
      <JsonLd data={localBusinessSchema("en")} />
      <JsonLd data={breadcrumbSchema([{ name: "Home", url: "/" }, { name: d.title, url: "/hotel-assault-negligent-security-lawyer" }])} />
      ...
      <section className="py-16">
        <div className="mx-auto max-w-3xl px-4">
          ...
          <div className="mt-6 overflow-hidden rounded-xl">
            <Image
              src={IMAGES.practice.hotel}
              alt="Hotel building with room key — premises liability"
              width={800}
              height={450}
      ...
      {/* Hazmat-specific section */}
      <section className="bg-gray-50 py-16">
        <div className="mx-auto max-w-3xl px-4">
          <h2 className="text-3xl font-bold text-brand-navy">{d.hazmatTitle}</h2>
          <p className="mt-4 text-lg leading-relaxed text-gray-600">{d.hazmatDesc}</p>
        </div>
      </section>
      ...
      <section className="py-10">
        <div className="mx-auto max-w-3xl px-4">
          ...
          <div className="mt-4 flex flex-wrap gap-3">
            <Link href={routes.sexualAssault} className="text-brand-red underline hover:text-brand-red-dark">Sexual Assault Lawyer</Link>
            <Link href={routes.rideshare} className="text-brand-red underline hover:text-brand-red-dark">Rideshare Sexual Assault Lawyer</Link>
            <Link href={routes.areas} className="text-brand-red underline hover:text-brand-red-dark">Areas We Serve</Link>
          </div>
        </div>
      </section>
