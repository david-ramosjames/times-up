...
export const metadata: Metadata = {
  title: "¿Cuál Es el Acuerdo Promedio en un Caso de Agresión Sexual?",
  description:
    "No existe un 'promedio' honesto: los resultados varían enormemente según el daño, los demandados y la evidencia. Obtenga una respuesta real. Evaluación confidencial gratis.",
  alternates: {
    canonical: `${SITE_URL}/es/ayuda-agresion-sexual/acuerdo-promedio-agresion-sexual`,
    languages: {
      en: `${SITE_URL}/sexual-assault-help/average-sexual-assault-settlement`,
      es: `${SITE_URL}/es/ayuda-agresion-sexual/acuerdo-promedio-agresion-sexual`,
    },
  },
};

export default function QAPageES() {
  const dict = getDictionary("es");
  const d = dict.helpHub;
  const q = d.questions[8];
  const routes = ROUTES.es;

  return (
    <PageShell dict={dict} locale="es">
      <JsonLd data={localBusinessSchema("es")} />
      <JsonLd data={breadcrumbSchema([{ name: "Inicio", url: "/es" }, { name: "Ayuda para Sobrevivientes", url: "/es/ayuda-agresion-sexual" }, { name: q.fullTitle, url: `/es/ayuda-agresion-sexual/acuerdo-promedio-agresion-sexual` }])} />
      ...
      <section className="bg-brand-navy py-16 text-white">
        <div className="mx-auto max-w-3xl px-4">
          <nav className="mb-6 text-sm text-gray-400">
            ...
            <Link href="/es/ayuda-agresion-sexual" className="hover:text-white">Ayuda para Sobrevivientes</Link>
            <span className="mx-2">/</span>
      ...
      <section className="py-16">
        <div className="mx-auto max-w-3xl px-4">
          ...
          <div className="mt-12 rounded-xl bg-brand-navy p-8 text-center text-white">
            <h2 className="text-2xl font-bold">¿Lista para Hablar? Aquí Estamos.</h2>
            <p className="mt-2 text-gray-300">Obtenga una consulta gratuita, confidencial y sin compromiso con nuestro equipo.</p>
          ...
          <div className="mt-12">
            ...
            <div className="mt-4 space-y-3">
              {d.questions.filter((_, i) => i !== 8).slice(0, 4).map((related) => (
            ...
            <div className="mt-6">
              <Link href="/es/ayuda-agresion-sexual" className="inline-flex items-center gap-1 font-medium text-brand-coral hover:text-brand-rose">
